...
class gui():
    ...
    def __init__(self, root, reader_process=None, model_process=None, Qin=None, Qcontrol=None, MAX_HIDS = 8, MIN_HIDS = 2, SKIP_HID = 2, IMG_W=244, IMG_H = 244, HID_W    = 30, HID_H    = 30, HID_GRID = 3):
        ...
        self.time_stat_names=[ "g8s", "g8u","g8c", "cc"]
        ...
        tmp_hids = im_tools.gallery(np.zeros((self.MAX_HIDS,self.HID_W,self.HID_H)), ncols=self.HID_GRID, nrows=self.HID_GRID)
        ...
        self.state_vars["Num Hids"].set(self.MAX_HIDS)
        '''
    ...
    def quit(self, *args):
        ...
        print('GUI: quit button press...')
        self.Qcontrol.put({'include_img': False, 'num_channels':8, 'quit':True})
        
        self.reader_process.join()
        self.model_process.join()

        print("GUI: all proccesses have quit")
    ...
    def do_refresh(self):
        ...
        if self.Qin.qsize()>0:
            ...
            if(frame_count>1):
                ...
                self.ax_time.set_ylim(-2, 1.2*np.max(np.sum(ys,axis=0)))
                plt.title("Process Times")
        ...
        self.root.after(200,self.do_refresh)
